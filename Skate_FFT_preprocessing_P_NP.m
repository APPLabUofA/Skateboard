...
if ~exist([Pathname 'segmentsFFT\'])
    mkdir([Pathname 'segmentsFFT_P_NP\']);
end
...
%                EEG = pop_eegfilt( EEG, 0, 30, [], 0);  %low pass filter
        ...
        EEG = pop_saveset( EEG, 'filename',ThisIsTheName,'filepath',[Pathname 'segmentsFFT_P_NP\']);
        
        if strcmp(conds{i_cond},'P_CCW') || strcmp(conds{i_cond},'P_CW')
            ThisIsTheName1 = [subs{i_sub} '_' exp '_' new_conds{1} '_Corrected_Standard.set']
        elseif strcmp(conds{i_cond},'NP_CCW') || strcmp(conds{i_cond},'NP_CW')
            ThisIsTheName1 = [subs{i_sub} '_' exp '_' new_conds{2} '_Corrected_Standard.set']
                    else
            ThisIsTheName = []
        end
        ...
        EEG = pop_saveset( EEG, 'filename',ThisIsTheName1,'filepath',[Pathname 'segmentsFFT_P_NP\']);
