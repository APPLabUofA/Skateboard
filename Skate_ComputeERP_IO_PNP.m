...
nsubs = length(subs);
conds =  {'preferred';'non-preferred';'facing_In';'facing_Out'};
conds_lab = {'Preferred Stance'; 'Non-Preferred Stance';'Facing Inside Track'; 'Facing Outside Track'};
nconds = length(conds);
...
for i_sub = 1:nsubs
    for i_cond = 1:nconds
        if strcmp (conds{i_cond}, 'preferred') || strcmp(conds{i_cond}, 'non-preferred')
            ...
            [ALLEEG, EEG, CURRENTSET] = eeg_store( ALLEEG, EEG, 0 );
        elseif strcmp(conds{i_cond},'facing_In') || strcmp(conds{i_cond},'facing_Out')
            Filename = [subs{i_sub} '_' exp '_' conds{i_cond}];
